...
#rl = 'https://servicebus2.caixa.gov.br/portaldeloterias/api/resultados?modalidade=Lotofácil'
url = sys.argv[1]

r = requests.get(url, verify=False)
#Tirando esse verify tem alguma falha de segurança, depois vou dar uma olhada nisso

conteudo = r.json()['html']

conteudo = conteudo.replace('\\r\\n','')
conteudo = conteudo.replace('\r\n','')

df = pd.read_html(conteudo)[0]
#vem uma lista com diversos dataframes, o que importa está na primeira posição

df = df[df['Bola1']==df['Bola1']].copy()
#nulo não é igual a ele mesmo
...
#contando a frequência que aparece um número, pares, impares e primos.
